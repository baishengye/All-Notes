...
![](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/4/10/162b00a94f6af409~tplv-t2oaga2asx-zoom-in-crop-mark:3024:0:0:0.awebp)
...
简单的说就是操作系统中，进程与进程间内存是不共享的。两个进程就像两个平行的世界，A 进程没法直接访问 B 进程的数据，这就是进程隔离的通俗解释。A 进程和 B 进程之间要进行数据交互就得采用**特殊的通信机制：进程间通信（IPC）**。

##### 进程空间划分：用户空间(User Space)/内核空间(Kernel Space)

现在操作系统都是采用的虚拟存储器，对于 32 位系统而言，它的寻址空间（虚拟存储空间）就是 2 的 32 次方，也就是 4GB。操作系统的核心是内核，独立于普通的应用程序，可以访问受保护的内存空间，也可以访问底层硬件设备的权限。为了保护用户进程不能直接操作内核，保证内核的安全，操作系统从逻辑上将虚拟空间划分为用户空间（User Space）和内核空间（Kernel Space）。针对 Linux 操作系统而言，将最高的 1GB 字节供内核使用，称为**内核空间**；较低的 3GB 字节供各进程使用，称为**用户空间**。

>简单的说就是，内核空间（Kernel）是系统内核运行的空间，用户空间（User Space）是用户程序运行的空间。为了保证安全性，它们之间是隔离的。

![](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/4/10/162b00a94fa38f32~tplv-t2oaga2asx-zoom-in-crop-mark:3024:0:0:0.awebp)
...
当一个任务（进程）执行系统调用而陷入内核代码中执行时，称进程处于**内核运行态（内核态）**。此时处理器处于特权级最高的（0级）内核代码中执行。当进程处于内核态时，执行的内核代码会使用当前进程的内核栈。每个进程都有自己的**内核栈**。
...
![](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/4/10/162b00a954b2c791~tplv-t2oaga2asx-zoom-in-crop-mark:3024:0:0:0.awebp)
...
这就不得不通道 Linux 下的另一个概念：**内存映射(mmap —— memory map)**。
...
2. 接着在内核空间开辟一块内核缓存区，**建立内核缓存区和内核中数据接收缓存区之间的映射关系**，以及**内核中数据接收缓存区和接收进程用户空间地址的映射关系**；
3. 发送方进程通过系统调用 `copy_from_user()` 将数据 `copy` 到内核中的内核缓存区，由于内核缓存区和接收进程的用户空间存在内存映射，因此也就相当于把数据发送到了接收进程的用户空间，这样便完成了一次进程间的通信。

如下图：
![](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/4/10/162b00a9557158d0~tplv-t2oaga2asx-zoom-in-crop-mark:3024:0:0:0.awebp)
...
![](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/4/10/162b00a955f25e1c~tplv-t2oaga2asx-zoom-in-crop-mark:3024:0:0:0.awebp)
...
![](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/4/10/162b00a979646aee~tplv-t2oaga2asx-zoom-in-crop-mark:3024:0:0:0.awebp)

[Android Binder 设计与实现一文](https://link.juejin.cn/?target=http%3A%2F%2Fblog.csdn.net%2Funiversus%2Farticle%2Fdetails%2F6211589)中对 Client、Server、ServiceManager、Binder 驱动有很详细的描述，以下是部分摘录：
...
![](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/4/10/162b00a98173c4c4~tplv-t2oaga2asx-zoom-in-crop-mark:3024:0:0:0.awebp)
...
![](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/4/10/162b00a98198b306~tplv-t2oaga2asx-zoom-in-crop-mark:3024:0:0:0.awebp)
...
![](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/4/10/162b00a98564d8db~tplv-t2oaga2asx-zoom-in-crop-mark:3024:0:0:0.awebp)
...
[Android Binder 设计与实现 - 设计篇](https://link.juejin.cn/?target=http%3A%2F%2Fblog.csdn.net%2Funiversus%2Farticle%2Fdetails%2F6211589)
[Android 进程间通信（IPC）机制 Binder 简要介绍和学习计划、《Android 系统源代码情景分析》](https://link.juejin.cn/?target=http%3A%2F%2Fblog.csdn.net%2Fluoshengyang%2Farticle%2Fdetails%2F6618363)
[Binder 学习指南](https://link.juejin.cn/?target=http%3A%2F%2Fweishu.me%2F2016%2F01%2F12%2Fbinder-index-for-newer%2F)
[Binder 系列文章](https://link.juejin.cn/?target=http%3A%2F%2Fgityuan.com%2F2015%2F10%2F31%2Fbinder-prepare%2F)
[Android 图文详解 Binder 跨进程通信原理](https://link.juejin.cn/?target=https%3A%2F%2Fblog.csdn.net%2Fcarson_ho%2Farticle%2Fdetails%2F73560642)
[Android 深入浅出之 Binder 机制](https://link.juejin.cn/?target=http%3A%2F%2Fwww.cnblogs.com%2Finnost%2Farchive%2F2011%2F01%2F09%2F1931456.html)
[用户空间与内核空间](https://link.juejin.cn/?target=http%3A%2F%2Fwww.cnblogs.com%2FAnker%2Fp%2F3269106.html)
[认真分析 mmap ：是什么 为什么 怎么用](https://link.juejin.cn/?target=https%3A%2F%2Fwww.cnblogs.com%2Fhuxiao-tee%2Fp%2F4660352.html)

```yml
作者：BaronTalk
链接：https://juejin.cn/post/6844903589635162126
来源：稀土掘金
