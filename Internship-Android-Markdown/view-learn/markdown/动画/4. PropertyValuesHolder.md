...
我们在使用ValueAnimator或者ObjectAnimator的时候有一个
ofPropertyValuesHolder()方法，里面的参数是PropertyValuesHolder。
可以帮助我们在一个animator中改变控件多个参数
...
```
都可以提前把目标和参数设置好然后直接在ValueAnimator或者ObjectAnimator中使用
...
```
**虽然有些参数在控件中存在get和set方法，但是也不能直接通过propertyName来设置使用,比如layoutParams**